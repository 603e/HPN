...
<script type="text/javascript">
	...
	var submitNow = function($dialog, $grid, $pjq) {
		...
		if ($(':input[name="data.id"]').val().length > 0) {
			url = frm.contextPath + '/base/frmUser!update.do';
		} else {
			url = frm.contextPath + '/base/frmUser!save.do';
		}
	...
	$(function() {
		...
		if ($(':input[name="data.id"]').val().length > 0) {
			...
			$.post(frm.contextPath + '/base/frmUser!getById.do', {
				...
				if (result.id != undefined) {
					...
					$('#orgName').text(result.frmOrganization.name);
					if (result.photo) {
						$('#photo').attr('src', frm.contextPath + result.photo);
					}
				}
		...
		uploader = new plupload.Uploader({//上传插件定义
			...
			//flash_swf_url : frm.contextPath + '/js/plupload_1_5_7/plupload/js/plupload.flash.swf',// Flash环境路径设置
			url : frm.contextPath + '/plupload?fileFolder=/userPhoto',//上传文件路径
