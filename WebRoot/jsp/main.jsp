	...
	$(function() {
		...
		var loginFun = function() {
			if ($('#loginDialog form').form('validate')) {
				...
				$.post(frm.contextPath + '/base/frmUser!doNotNeedSessionAndSecurity_login.do', $('#loginDialog form').serialize(), function(result) {
					if (result.success) {
			...
			buttons : [ {
				...
				handler : function() {
					if ($('#passwordDialog form').form('validate')) {
						$.post(frm.contextPath + '/base/frmUser!doNotNeedSecurity_updateCurrentPwd.do', {
							'data.pwd' : $('#pwd').val()
						}, function(result) {
							if (result.success) {
		...
		mainMenu = $('#mainMenu').tree({
			url : frm.contextPath + '/base/frmResource!doNotNeedSecurity_getMainMenu.do',
			parentField : 'pid',
			onClick : function(node) {
				if (node.attributes.url) {
					var src = frm.contextPath + node.attributes.url;
					if (!frm.startWith(node.attributes.url, '/')) {
					...
					if (node.attributes.target && node.attributes.target.length > 0) {
						...
						var opts = {
							...
							content : frm.formatString('<iframe src="{0}" allowTransparency="true" style="border:0;width:100%;height:99%;" frameBorder="0"></iframe>', src),
							border : false,
							fit : true
						};
		...
		$('#mainLayout').layout('panel', 'center').panel({
			onResize : function(width, height) {
				frm.setIframeHeight('centerIframe', $('#mainLayout').layout('panel', 'center').panel('options').height - 5);
			}
		});
...
<body id="mainLayout" class="easyui-layout">
	<div data-options="region:'north',href:'<%=contextPath%>/jsp/north.jsp'" style="height: 70px; overflow: hidden;" class="logo"></div>
	...
	<div data-options="region:'center'" style="overflow: hidden;">
		<div id="mainTabs">
			<div title="主页" data-options="iconCls:'ext-icon-heart'">
				<iframe src="<%=contextPath%>/welcome.jsp" style="border: 0; width: 100%; height: 99%; allowTransparency:true; frameBorder:0"></iframe>
			</div>
		</div>
	</div>
