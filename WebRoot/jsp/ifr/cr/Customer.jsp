...
<script type="text/javascript">
	...
	var addFun = function() {
		var dialog = parent.frm.modalDialog({
			title : '添加用户信息',
			url : frm.contextPath + '/jsp/ifr/cr/CustomerForm.jsp',
			buttons : [ {
				text : '添加',
				handler : function() {
	...
	var showFun = function(id) {
		...
		var dialog = parent.frm.modalDialog({
			title : '查看用户信息',
			url : frm.contextPath + '/jsp/jsp/ifr/cr/CustomerForm.jsp?id=' + rowId
		});
	};
	var editFun = function(id) {
		...
		var dialog = parent.frm.modalDialog({
			title : '编辑用户信息',
			url : frm.contextPath + '/jsp/ifr/cr/CustomerForm.jsp?id=' + rowId,
			buttons : [ {
				text : '更新',
				handler : function() {
					dialog.find('iframe').get(0).contentWindow.submitForm(dialog, grid, parent.$);
	...
	var removeFun = function(id) {
		...
		parent.$.messager.confirm('询问', '您确定要删除此记录？', function(r) {
			if (r) {
					$.post(frm.contextPath + '/ifr/cr/customer!delete.do', {
						id : rowId
					}, function() {
	...
	$(function() {
		grid = $('#grid').datagrid({
			title : '',
			url : frm.contextPath + '/ifr/cr/customer!grid.do',
			striped : true,
			rownumbers : true,
			pagination : true,
			singleSelect : true,
...
<body class="easyui-layout" data-options="fit:true,border:false">
	...
	<div id="toolbar" >
		<table>
			<tr>
			<%if (securityUtil.havePermission("/ifr/cr/customer!save")) {%>
				<td><a href="javascript:void(0);" class="easyui-linkbutton" data-options="iconCls:'ext-icon-note_add',plain:true" onclick="addFun();">添加</a></td>
			<%}%>
			<%if (securityUtil.havePermission("/ifr/cr/customer!getById")) {%>
			...
			<%}%>
			<%if (securityUtil.havePermission("/ifr/cr/customer!update")) {%>
				<td><a href="javascript:void(0);" class="easyui-linkbutton" data-options="iconCls:'ext-icon-note_add',plain:true" onclick="editFun();">编辑</a></td>
			<%}%>
			<%if (securityUtil.havePermission("/ifr/cr/customer!delete")) {%>
